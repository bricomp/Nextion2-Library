...
void DisplayInBox(uint8_t x, uint8_t y, const char* text, uint32_t before, uint32_t after) {
	...
	display.setVariableValue("page0.CommentBox", setX, x);

	delay(d); display.printAnyReturnCharacters(nextionTime, 100);
	display.setVariableValue("page0.CommentBox", setY, y);

	delay(d); display.printAnyReturnCharacters(nextionTime, 101);
	display.setVariableValue("page0.CommentBox",text);
...
/*
	display.gotoPage(2);
	if (display.setVariableValue("t0", "About to read and then set a Float to x0")) {
	...
	display.setVariableValue("t0", "Seting Float x0 to 1234.5678 to 2dp rounded");
	display.setNumVarFloat("x0", 1234.5678, 2, true);
	delay(5000);
	display.setVariableValue("t0", "Seting Float x0 to 1234.5678 to 2dp NOT rounded");
...
*/
	...
	*********************************************************************/
	...
	display.setVariableValue("debugTxt", "Here we are Debug Text");
	display.viewDebugText(true);
	display.setVariableValue("debugTxt", setXcen, 0);
	delay(1000);
...
//		if (n == 1) {
			Serial.print(display.getVariableValue("HWDwnCtr")); Serial.print(" - ");
//		}
		Serial.println(nextionTime);
